...
function createClient(req, res) {
    ...
    ClientModel.find({ userName: query.userName }, (err2, data1) => {
        if (data1.length > 0) {
            res.status(200).send({ status: false, message: 'El nombre de usuario ya existe' });
        } else {
...
function listClient(req, res) {
    ClientModel.find({ status: true }).sort({ created_at: -1 }).exec((err, data) => {
        if (err) {
            res.status(200).send({ status: false, message: 'Fallo al listar los datos' });
        } else {
...
function updateClient(req, res) {
    ...
    ClientModel.findByIdAndUpdate(update.id, update, (err, data) => {
        if (err) {
            res.status(200).send({ status: false, message: 'Fallo al actualizar los datos' });
        } else {
